<%-- 
    Document   : checkoutLogin
    Created on : 25-abr-2011, 21:06:25
...
<html>
	...
	<div id="container">
            <div id="center" class="column"><br />
                ...
                <div id="content">
                    <div id="about">
                        <p class="tree"><b>Pulse el ACEPTAR para confirmar la compra:</b></p>
                        <br /><p></p> <%if(info!=null && !info.equals("")){%> <p class="line centerred"><%=info%></p><br /> <%}%>
                        <form method="post" action="/TiendaOnline/go">
                            <input name="to" value="checkout" type="hidden">
                            <input name="action" value="pay" type="hidden">
                            <p class="line center pad20"><button>ACEPTAR</button>
                        </form>
                    </div>
                 </div>
                <%}else { %>
                <div id="content">
                    <div id="about">
                        <p class="tree"><b>Escriba sus datos para loguearse y efectuar el pago:</b></p>
                        <br /><p></p>
                        <form method="post" action="/TiendaOnline/go">
                            <input name="to" value="user" type="hidden"><pre>
                            <input name="from" value="checkout" type="hidden"><pre>
                            ...
                            <p class="line centerred"><%if(loginError != null && !loginError.equals("")) {%><%=loginError%><%} %></p></pre>
                            <p class="line center pad20"><button>Loguearse</button>
                        </form>
                    </div>
                </div>
                <div id="content">
                    <div id="about">
                        <p class="tree"><b>O si no posee cuenta rellene el formulario:</b></p>
                        <br /><p></p>
                        <form method="post" action="/TiendaOnline/go">
                            <input name="to" value="checkout" type="hidden">
                            <input name="action" value="pay" type="hidden"><pre>
                        <p class="line"><span>Nombre completo:      </span><input name="customerName" value="" type="text" /></p>
                        <p class="line"><span>Direccion de envio:  </span> <input name="address" value="" type="text" /></p>
                        
                        <br/><p class="line"><span>Forma de Pago:<br/></span><select name="paymentForm"><option>Tarjeta</option><option>Contra reembolso</option><option>La voluntad</option></select>
                        </p></pre>
                        <p class="liner"><button>Comprar</button></p>    
                        <%if(info!=null && !info.equals("")){%> <p class="line centerred"><%=info%></p><br /> <%}%>
                    </form>
                    </div>
                </div>
                <%} %>
