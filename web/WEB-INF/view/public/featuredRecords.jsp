<%-- 
    Document   : featuredRecords
    Created on : 07-may-2011, 18:48:23
...
<html>
	...
	<div id="container">
            <div id="center" class="column"><br />
                <div id="content">
                    <div id="about">
                        <p class="tree"><b>Los 10 discos mas vendidos</b></p>
                        <div id="record">
                        <%Record[] featuredRecords = catalog.getFeaturedRecords();
                        int addPad = 0;
                        for(int i=0;i<featuredRecords.length;i++) {
                          if(featuredRecords[i] != null) {%>
                            <div class="photos">
                                <%if((addPad%2)==0) {%>
                                    <pre>
                            ...
                            <div class="description">																																																																																																																																																			<div class="inner_copy"><a href="http://www.bestfreetemplates.org/">free templates</a><a href="http://www.bannermoz.com/">banner templates</a></div>
                                <a href="/TiendaOnline/go?to=product&id=<%=featuredRecords[i].getIdAsString()%>" class="name"><%=featuredRecords[i].getName()%></a>
                                <pre>
                                
                                </pre>
                                <a href="/TiendaOnline/go?to=search&artist=<%=featuredRecords[i].getArtist()%>" class="artist"><%=featuredRecords[i].getArtist()%></a>
                                <p> <span class="price"><%=featuredRecords[i].getPriceAsFormattedString()%></span></p>
                                <p><%=featuredRecords[i].getShortComment()%></p>
                                <a href="/TiendaOnline/go?to=shoppingcart&id=<%=featuredRecords[i].getIdAsString()%>&n=1"><img src="cssimages/carts.gif" alt="" width="16" height="24" /></a>
                                <%if(admin!=null && admin.equals("true")) {%>
                                    <a href="/TiendaOnline/go?to=admin&action=deleterecord&id=<%=featuredRecords[i].getIdAsString()%>" onclick="return confirm('El disco se borrará, OK?');"><img src="cssimages/delete.gif" alt="" width="24" height="24" /></a>
                                <%}%><pre>
