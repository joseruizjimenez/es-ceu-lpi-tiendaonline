<%-- 
    Document   : adminRecordUpdate
    Created on : 25-abr-2011, 21:24:22
...
<html>
	...
	<div id="container">
            <div id="center" class="column">
                <div id="content">
                    <div id="about">
                        <p class="tree"><b>Modifique los datos para actualizar el disco:</b></p><br/>                  
                    <form method="post" action="/TiendaOnline/go">
                        <%Record record = catalog.getRecord(id); %>
                        <input name="to" value="admin" type="hidden">
                        <input name="action" value="updaterecord" type="hidden">
                        <input name="id" value="<%=id %>" type="hidden"><pre>
                        <p class="line"><span>Nombre del disco:             </span><input name="name" value="<%=record.getName() %>" type="text" /></p>
                        <p class="line"><span>Artista/Autor:               </span> <input name="artist" value="<%=record.getArtist() %>" type="text" /></p>
                        <p class="line"><span>Discografica:                </span> <input name="recordLabel" value="<%=record.getRecordLabel() %>" type="text" /></p>
                        <p class="line"><span>Descripcion corta (140 max): </span> <input name="shortComment" value="<%=record.getShortComment() %>" type="text" maxlenght="140" /></p>
                        <p class="line"><span>Descripcion (300 max):       </span></p>




 
 <textarea name="fullComment" cols="43" rows="6"><%=record.getFullComment() %></textarea>
                        <p class="line"><span>Categoria:                   </span> <input name="type" value="<%=record.getType() %>" type="text" /></p>
                        <p class="line"><span>Precio (decimales con '.'):  </span> <input name="price" value="<%=record.getPriceAsString() %>" type="text" /></p>
                        <p class="line"><span>Unidades de stock:            </span><input name="stock" value="<%=catalog.getStock(id) %>" type="text" size="4" maxlength="4" /></p>
                        </pre>
                        <p class="liner"><button>Actualizar</button></p>
