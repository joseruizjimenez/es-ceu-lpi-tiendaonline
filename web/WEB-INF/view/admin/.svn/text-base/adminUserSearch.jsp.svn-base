<%-- 
    Document   : adminUserSearch
    Created on : 25-abr-2011, 21:22:02
    ...
    <head>
        ...
        <script>
            $(document).ready(function() {
                $("#users").jPaginate({previous:"Anterior", next:"Siguente"});
            });
        </script>
    </head>
    <body>
                ...
                <div id="content">
                    <div id="about">
                        ...
                        <form method="post" action="/TiendaOnline/go">
                            ...
                            <input name="action" value="searchuser" type="hidden"><pre>
                            ...
                            <br/><p class="line"><span>Fecha de Nacimiento:<br/></span> Dia: <select name="birthDay"><option>0</option><%for(int i=1;i<=31;i++){%><option><%=i%></option><%}%></select>&nbsp;&nbsp;Mes: <select name="birthMonth"><option>0</option><%for(int i=1;i<=12;i++) {%><option><%=i%></option><%}%></select>&nbsp;&nbsp;Año: <select name="birthYear">
                                    <option>0</option><%for(int i=2011;i>1940;i--) {%><option><%=i%></option><%}%></select>
                            </p></pre>
                        ...
                        <%if(list!=null && !list.isEmpty()) {
                           ...
                           for(Object obj : list) {
                            ...
                            <%if((addPad%2)==0) {%>
                                ...
                                <%}%>
                            <pre>                                                       
                                <p class="line"><span><b><%=userFound.getLastName() %>, <%=userFound.getFirstName() %></b></span></p>
                                <p class="line"><span><b>-----------------------------------------------------------------</b></span></p>                              
                                <p class="line"><span><b>Nick: </b><%=userFound.getNick() %></span></p>


                               
                                <p class="line"><span><b>Password: </b><%=userFound.getPassword() %></span></p>                               
                                <p class="line"><span><b>Direccion: </b><%=userFound.getAddress() %></span></p>
                                
                                <p class="line"><span><b>Forma de pago: </b><%=userFound.getPaymentForm() %></span></p>
                          
                                <p class="line"><span><b>Email: </b><%=userFound.getEmail() %></span></p>

                                <p class="line"><span><b>Fecha de Nacimiento: </b><%=userFound.getBirthDateAsString() %></span></p>
