...
public class UserUpdateServlet extends BasicUtilitiesServlet {
    Logger logger = Logger.getLogger(UserUpdateServlet.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (isBothPasswordEqual(request) && isUserFormSanitized(request)) {
            ...
            if(!oldUser.getNick().equals(nick) && isNickTaken(userDAO,nick)) {
                request.setAttribute("registerError","Ese nick: "+nick+" ya esta cogido");
            } else {
                
                User newUser = generateUpdatedUserFromRequest(oldUser.getIdAsString(),request);
                if(userDAO.updateUser(oldUser.getIdAsString(),newUser)) {
                    request.setAttribute("info","¡Su cuenta ha sido actualizada con exito!");
                    request.setAttribute("mail","true");
                    session.setAttribute("user",newUser);
                    logger.info("Actualiza la cuenta de usuario: "+newUser.getIdAsString());
                    ...
                    logger.warn("Error insertando la cuenta de usuario: "+newUser.getIdAsString());
                }
            }
        } else {
     ...
     */
    private User generateUpdatedUserFromRequest(String id, HttpServletRequest request) {
        String nick = request.getParameter("nick");
        String pass = request.getParameter("pass1");
        String firstName = request.getParameter("firstName");
        String lastName = request.getParameter("lastName");
        ...
        return new User(id,nick,pass,firstName,lastName,address,email,year,month,day);
    }
